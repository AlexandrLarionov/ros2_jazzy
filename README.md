...
Изображение запущенной сцены и модели робота

![Фотография робота Mecanum](photo/screen_2.png)

![Фотография rqt graph](photo/frames.jpg)
...
7. Запустите скрипт и выберите стол, чтобы тележка к нему перемещалась

* ros2 run yahboom_rosmaster_navigation test_nav_to_pose.py --ros-args -p use_sim_time:=true

## 0. Структура проекта (ROS 2 Workspace)

Рабочее пространство ROS 2 настроено по референсным стандартам сообщества и разделено на логические назначения пакетов (отдельные структуры для запуска, хранения файлов моделей, скриптов управления. тестовых прогонов).

### 0.1. Пакеты управления кинематикой
...
### 0.3. Пакеты моделирования и симулятора
...
### 0.4. Пакеты локализации и навигации (Nav2)

| Пакет | Описание | Ключевые файлы и папки |
| :--- | :--- | :--- |
| **`yahboom_rosmaster_localization`** | **Локализация и Сенсорное Слияние.** Содержит настройки для повышения точности оценки позы робота, является частью Navigation2. | **`config/ekf.yaml`**: дефолтный YAML файл с настройками расширенного Фильтра Калмана (EKF) для слияния данных одометрии и IMU, очистки от шумов и точного позиционирования для оценки позы робота - брался готовый, корректировал некоторые параметры такие как время ожидания данных, нужный фрейм для расчета. Содержит информацию об обрабатываемых данных, частоте, публикации данных. |
| **`yahboom_rosmaster_navigation`** | **Высокоуровневая Навигация (Nav2).** Содержит конфигурацию для построения карты, планирования пути и автономного движения. | **`config/rosmaster_x3_nav2_default_params.yaml`**: Базовые параметры стека Nav2 (Costmap, Global map, AMCL, Planners, Controllers control server) - брал базовые значения, в некоторых частях меня частоту обновления сообщений, параметры сервера, фрейма.<br>**`maps/`**: Директория для хранения карт. <br>**`src/cmd_vel_relay_node`** - узел ретранслятор сообщений cmd_vel на новый узел для mecanum_drive_controller, так как nav2 использует его для своих публикаций.|


### Ключевые файлы системы списком (не все launch, основные с параметрами):

* mecanum_drive_controller - контроллер из https://github.com/ros-controls/ros2_controllers, публикует в mecanum/odom
* rosmaster_x3_navigation.launch.py - лаунч файл для установки всех компонент и запуска окружения
* rosmaster_x3_navigation.sh - bash файл для последовательного запуска команд
* robot_state_publisher.launch.py - запускает rviz, устанавливает параметры файлов модели и контроллеров
* ros_gz_bridge.yaml - мост, прокидывает основные топики, связывает время и данные
* ekf.yaml - основной файл EKF для локализации
* rosmaster_x3_nav2_default_params.yaml - файл конфигурации для nav2, содержит в себе информацию по amcl, bt_navigator, local и global costmap
...
Эти уравнения используются в контроллере движения ROS 2 для перевода команд скорости в фактическое движение колёс.
...
Прямая задача кинематики вычисляет движение робота на основе угловых скоростей колёс. Эти скорости публикуются в сообщении `Twist` как линейные и угловые скорости.
...
Обратная задача кинематики вычисляет угловые скорости каждого колеса, необходимые для достижения желаемого движения робота, которое обычно отправляется через топик `/cmd_vel`.
